...
ListNode* getListNode(List* list, U64 index) {
  ...
  if (index >= list->size) {
    LOG_WARNING("Provided index larger than size");
    return NULL;
  }
...
List* splitList(List* list, U64 index) {
  ...
  newList->size = list->size - index - 1;
...
ListInfo* createListInfo(void* (*createItemInfo)(void*, va_list), void (*debugItemInfo)(void*)) {
  ...
  ISNULL(info)
  ISNULL(createItemInfo)
  ISNULL(debugItemInfo)
  info->createItemInfo = createItemInfo;
  info->debugItemInfo = debugItemInfo;
